...
class DatasetConfig:
    ...
    """Path to h5ad file."""
    hvgs_fpath: pathlib.Path
    """Path the hvgs.json file."""
...
class Block(eqx.Module):
    ...
    def __call__(self, x_sd: Float[Array, "set d"], *, key: chex.PRNGKey):
        # x_sd: (S, d), self-attention over the S cells
        q_sd = k_sd = v_sd = jax.vmap(self.ln1)(x_sd)
        x_sd = x_sd + self.attn(q_sd, k_sd, v_sd, inference=False, key=key)
        x_sd = x_sd + jax.vmap(self.mlp)(jax.vmap(self.ln2)(x_sd))
        return x_sd
...
class Transformer(eqx.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.f_cell = eqx.nn.MLP(n_genes, d, d, depth=2, activation=jax.nn.gelu, key=k1)
        self.emb_pert = eqx.nn.Embedding(
            num_embeddings=n_perts, embedding_size=d, key=k2
        )
        self.f_pert = eqx.nn.MLP(d, d, d, depth=2, activation=jax.nn.gelu, key=k3)
        self.blocks = [
            Block(d, n_heads, mlp_mult, key=jr.fold_in(k4, i)) for i in range(n_layers)
        ]
        # W_recon and b_recon in the state apper.
        self.head = eqx.nn.Linear(d, n_genes, key=k5)
    ...
    def __call__(
        ...
        *,
        ...
        x_sd = jax.vmap(self.f_cell)(x_sg)
        p_d = self.f_pert(self.emb_pert(pert_id))
        x_sd = x_sd + jnp.broadcast_to(p_d, x_sd.shape)

        for i, block in enumerate(self.blocks):
            x_sd = block(x_sd, key=jr.fold_in(key, i))

        delta_sg = jax.vmap(self.head)(x_sd)
...
def step_model(
    ...
    (loss, metrics), grads = eqx.filter_value_and_grad(loss_and_aux, has_aux=True)(
        model, ctrls_bsg, perts_b, tgts_bsg, mask_bg, key
    )
...
class LoadAndLift(grain.transforms.Map):
    ...
    def get_stuff_for_loading(self, cfg: DatasetConfig) -> StuffForLoading:
        if cfg not in self._stuff_for_loading:
            ...
            # GeneMap
            gmap = self.gene_vocab.make_map(adata, gene_id_col=cfg.gene_id_col)

            # HVGs
            with open(cfg.hvgs_fpath) as fd:
                hvg_symbols = [
                    symbol
                    for symbol, fields in json.load(fd).items()
                    if fields["highly_variable"]
                ]
            hvgs = [i for i in adata.var_names.get_indexer(hvg_symbols) if i >= 0]
            hvgs = np.array(hvgs[:2000])

            self._stuff_for_loading[cfg] = StuffForLoading(adata, gmap, hvgs)

        return self._stuff_for_loading[cfg]

    def map(self, sample: Sample):
        cfg = sample["cfg"]
        adata, gmap, hvgs = self.get_stuff_for_loading(cfg)
